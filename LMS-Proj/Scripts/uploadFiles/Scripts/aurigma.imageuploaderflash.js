...
AU.debug.prototype = {
    ...
    version: function () {
      return AU.Control.Uploader.prototype.getVersion();
    },
    buildDate: function () {
      return AU.Control.Uploader.prototype.getBuildDate();
    },
    mode: function (mode) {
        ///	<summary>
...
extend(AU.events.prototype, {
    _eventNames: [undefined /*base event*/, undefined /*base event*/, 'beforeUpload', 'afterUpload', 'beforePackageUpload',
        'afterPackageUpload', 'progress', 'error', 'restrictionFailed', 'addingFiles', 'trace', 'uploadFileCountChange','selectionChange','ImageEditorClose'],
...
AU.file.reinit = function (control, obj, type) {
    ...
    if (type == 'html') {
        ...
        obj.getPreview = function (params) {
            return control.getUploadFilePreviewSrcAt(this._index, params.callback.bind(obj), params.size);
        };
	...
	obj.showImageEditor = function(){
		control.showImageEditorById(this.guid());
	};
...
AU.files.reinit = function (obj) {
    ...
    if (type === 'html') {
        ...
        obj.add = function (name, file) {
            if(typeof file == "string"){
              control.addUploadFilesAsBase64([[name, file]]);
            }else if(name, file){
              control.addUploadFilesAsBlob([[name, file]]);
            }
        };
    } else if (type === 'flash') {
        obj.count = function () {
...
AU.metadata.prototype = {
    ...
    _simpleProperties: [
        ...
        { name: "enableAllStandardFields", getter: 1, setter: 1, render: "EnableAllStandardFields" }
    ],
...
AU.uploadSettings.prototype = {
    ...
    redirectUrl: function (url) {
		...
		for(var i=0;handlers.length;i++){
			if(handlers[i] == this._afterUpload){
		...
		if(!isRedirectConfigured){
			...
			this._uploader.events().afterUpload().add(this._afterUpload);
		} 
        } else {
...
extend(AU.imageUploaderFlash.fn, {
    ...
    version: function () {
      if (this.type() === 'html')
        return this.getControl().getVersion();
    ...
    buildDate: function () {
      if (this.type() === 'html')
        return this.getControl().getBuildDate();
      else if(this.type() === 'flash')
...
if(window.$au)
window.$au.imageUploaderFlash = $au.imageUploaderFlash;
